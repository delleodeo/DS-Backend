...
exports.createReviewService = async (reviewData) => {
	try {
		...
		if (!order) {
			console.error('❌ [REVIEW] Order not found:', orderId);
			throw new Error("Order not found");
		}

		console.log('📦 [REVIEW] Order found:', {
			orderId: order._id,
			status: order.status,
		...
		if (order.customerId.toString() !== userId.toString()) {
			console.error('❌ [REVIEW] User mismatch. Order customer:', order.customerId, 'Review user:', userId);
		...
		if (!orderItem) {
			console.error('❌ [REVIEW] Product not in order. Looking for:', productId, 'Order items:', order.items.map(i => i.productId));
...
// GET PRODUCT REVIEWS
exports.getProductReviewsService = async (productId, page = 1, limit = 10, sortBy = "createdAt") => {
	try {
...
// VENDOR RESPONSE
exports.addVendorResponseService = async (reviewId, vendorId, responseComment) => {
	try {
		const review = await Review.findById(reviewId);
